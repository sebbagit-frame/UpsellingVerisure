...
import { BUCKET_DISPOSITIVOS } from "@/lib/storageDispositivos";
...
function generarNombreArchivo(nombreOriginal: string): string {
  const saneado = nombreOriginal
    ...
    .replace(/-+/g, "-")
    .replace(/^-|-$/g, "");
  return `${Date.now()}-${saneado || "imagen"}`;
}
...
export async function POST(solicitud: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json(
      { error: "Se esperaba un formulario con el archivo de imagen" },
      { status: 400 }
    );
  }
